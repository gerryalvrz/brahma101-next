...
- **Art (`/art`):** Live `hydra-synth` canvas (not a fork of the full hydra editor). Sketches live in `src/data/art.ts`; UI in `src/components/art/HydraArt.tsx`. Client-only; `detectAudio: false` until toolbar audio. Hydra Assist chat: local quick-fixes + Venice via `POST /api/hydra/assist` (`VENICE_API_KEY`). Docs pack in `src/data/hydraAssistDocs.ts`.
