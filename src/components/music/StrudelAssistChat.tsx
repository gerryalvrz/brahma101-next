...
import { localQuickFix } from "@/lib/strudel/quick-fix";
import styles from "./StrudelAssistChat.module.css";
...
export default function StrudelAssistChat({
  code,
  error,
  uiVisible,
  onApply,
  ...
  const [messages, setMessages] = useState<AssistMessage[]>([
    {
      id: "welcome",
      role: "system",
      content: "brahma101 · your helper. welcome to strudel.",
    },
  ]);
  ...
  async function askVenice(message: string) {
    ...
    const res = await fetch("/api/strudel/assist", {
      method: "POST",
  ...
  async function submit(question: string, mode: "ask" | "fix") {
    ...
    try {
      const local = localQuickFix({ code, error });
  ...
  function onFix() {
    void submit(
      error
        ? `Fix this Strudel pattern. Runtime error: ${error}`
        : "Fix or improve this Strudel pattern so it plays cleanly.",
      "fix"
    );
  }
  ...
  return (
    <aside
      ...
      aria-label="brahma101 strudel helper"
